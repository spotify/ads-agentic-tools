...
| Media plan to draft | “Turn this agency media plan into a Spotify draft campaign.” | “Process every relevant tab in `<third_party_plan.xlsx>`, reconcile totals, resolve targets and assets, run forecasts, and wait for approval before drafting; the CRM audiences are not uploaded yet.” | `media-plan-to-draft` → `campaign-strategy`/`assets` → review gate → `drafts`; source provenance; accepted incompleteness; never publish |
