...
- **Session header**: Every API request must include `X-Spotify-Ads-Session: <uuid>`. Generate a fresh UUID once at the start of each agent conversation: `SESSION_ID=$(uuidgen | tr '[:upper:]' '[:lower:]')` and set `SESSION_HEADER="X-Spotify-Ads-Session: $SESSION_ID"`. Reuse the same `SESSION_ID` for all requests within that conversation. This enables exact session metrics and distinguishes interactive use from automated scripts.
