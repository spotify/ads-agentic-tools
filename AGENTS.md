...
- **Hooks** — Per-platform hook configs invoking `hooks/check-token.sh` to automatically refresh expired OAuth tokens before Spotify API calls. `hooks/antigravity-hooks.json` contains the Antigravity CLI `PreToolUse` event (users copy it to `.agents/hooks.json` after install — see ANTIGRAVITY.md). `.claude-plugin/hooks.json` and `.codex-plugin/hooks.json` contain the Claude/Codex `PreToolUse` event, declared via the `hooks` field in each platform's `plugin.json`. The Antigravity hooks ship under a non-default filename to prevent Claude Code from auto-loading them.
