...
- **Scripts** (`scripts/`) — Shared shell scripts used by skills and agents:
  - `scripts/api-request.sh` — Request wrapper that handles settings discovery, authentication, SDK/skill tracking headers, and curl execution. Skills define a local `api()` function that delegates to this script: `api() { "$PLUGIN_ROOT/scripts/api-request.sh" <skill-name> "$@"; }`. Usage: `api <METHOD> <path> [json_body]` or `api --env` for settings. Paths use `{ad_account_id}` as a placeholder (auto-substituted from settings). `--env` prints `TOKEN`, `AD_ACCOUNT_ID`, `AUTO_EXECUTE`, `BASE_URL`, `SDK_HEADER`, `PLUGIN_VERSION`, and (when a skill name is passed) `SKILL_HEADER`. Every value is single-quoted so `eval $(api --env)` assigns them intact — unquoted, the space inside `SDK_HEADER` splits the assignment and `eval` silently sets nothing at all.
  ...
  - `skills/configure/scripts/settings_file.py` — Secure settings writer used during OAuth and direct-token setup. It creates replacement files as mode 0600 before atomically renaming them and never prints token values.
- **Hooks** — Per-platform hook configs invoking `hooks/check-token.sh` before Spotify API calls, to refresh expired OAuth tokens and to enforce telemetry attribution (see the next bullet). `hooks.json` at the plugin root contains the Antigravity `PreToolUse` event, auto-discovered by both Antigravity CLI (`agy plugin install`) and Antigravity 2.0 (`.agents/plugins/` workspace discovery). `.claude-plugin/hooks.json` and `.codex-plugin/hooks.json` contain the Claude/Codex `PreToolUse` event, declared via the `hooks` field in each platform's `plugin.json`. The Codex hook command uses Codex's `${PLUGIN_ROOT}` manifest substitution; do not replace it with a cwd fallback or assume `CODEX_PLUGIN_ROOT` is exported to the hook process. Note: the hook payload and response formats differ across platforms — Claude/Codex use `.tool_input.command` and support command rewriting via `updatedInput`, while Antigravity uses `.toolCall.args.CommandLine` and only supports allow/deny with `decision`/`reason` (the hook refreshes the token in the settings file and tells the agent to re-read it).
- **Hook attribution enforcement** (`hooks/lib/attribution.sh`, sourced by `hooks/check-token.sh`) — `scripts/api-request.sh` sets `X-Spotify-Ads-Skill` and `X-Spotify-Ads-Sdk` deterministically, but hand-written `curl` bypasses it, leaving those requests unattributable in per-skill usage and error-rate reporting. The hook closes that gap:
  - **Never register attribution as a second hook.** Matching `PreToolUse` hooks run in parallel, each receiving the *original* input, and the last `updatedInput` to finish wins in non-deterministic order. A second rewriting hook would race the token refresh and silently drop one of the two edits. There are also open upstream bugs where `updatedInput` is dropped once multiple `PreToolUse` hooks are involved. One hook owns the single rewrite; attribution is a sourced library, not a separate registration. For the same reason a user's own unrelated `PreToolUse` Bash hook can cause our rewrite to be dropped, in which case behaviour degrades to the `systemMessage` nudge, so keep that path working.
  - **Interface** — `apply_attribution <original> <current> <platform> <transcript>` sets `ATTRIBUTION_COMMAND` and `ATTRIBUTION_NOTE`. It reads `PLUGIN_ROOT` and never fails the caller. If the library is missing, the hook falls back to token refresh only.
  - **Detection** — claims a command naming `api-partner.spotify.com`, or referencing `$BASE_URL` together with a Spotify marker (`X-Spotify-Ads-`, `SDK_HEADER`, `SKILL_HEADER`, or `ad_accounts`). The `$BASE_URL` form matters because the assets and audiences upload flows never spell out the host. A command already counts as attributed if it names the header, references `$SKILL_HEADER`/`$SDK_HEADER`, or delegates to `api-request.sh`.
  - **SDK header** — deterministic, so it is injected verbatim when missing. The value comes from `api-request.sh --env` rather than a second copy of the platform/version logic, so it cannot drift from what skills send.
  - **Skill header** — can only be inferred. The hook scans the last `SPOTIFY_ADS_SKILL_LOOKBACK_LINES` (default 300) transcript lines newest-first, so recency beats pattern strength, and accepts the first candidate matching a real `skills/<name>/SKILL.md` or the request-builder agent. Matching is deliberately plain-text so the same patterns work across Claude, Codex, and Antigravity transcripts without encoding any one platform's JSON schema.
  - **Inferred values carry an `-inferred` suffix** (`campaigns-inferred`). Reporting can then treat exact matches as deterministic and `*-inferred` as best-effort. Never strip this suffix: a stale inference labelled as deterministic silently corrupts per-skill error rates.
  - **When inference fails** no skill header is injected and the agent gets a nudge toward `api()` instead. A sentinel would add nothing, since a null skill already reads as unattributed downstream. Note that the SDK header is still backfilled, so a call omitting both headers becomes visible with a null skill rather than staying invisible.
...
Before using the wrapper, each workflow fetches the current public OpenAPI document once and uses it to check the complete operation contract as described in `skills/api-reference/references/live-openapi.md`. The wrapper then handles settings discovery (platform-ordered fallback), authentication, SDK/skill tracking headers, and `\nHTTP_STATUS:<code>` capture without downloading the document again for each request. When a 403 response body says the client ID is not allow-listed (observed shape: `Client ID <id> is not allow-listed`), it also appends an `ADS_API_HINT` line pointing at the API terms page (https://adsmanager.spotify.com/api-terms) that allow-lists the client ID. If `auto_execute` is false, the skill presents the command and asks for confirmation before executing. Exceptions (asset file uploads, OAuth token exchange) use raw curl — see the relevant skill for details.
...
- **403 whose body says the client ID is not allow-listed**: The client ID is not allow-listed for the Ads API. Always tell the user to open https://adsmanager.spotify.com/api-terms with the target ad account selected and accept the API terms (the wrapper appends an `ADS_API_HINT` line with this URL; relay it). Saved tokens stay valid — retry after the terms are accepted. Do not describe this as a developer-dashboard access request.
- **Other 403s** (wrong ad account, insufficient role, data the token is not authorized for): ordinary permission denials — report the specific denial from the response body. Do not attach allow-list guidance to them.
