...
- **Hooks** (`hooks/` and `.claude-plugin/hooks.json`) — Two per-platform hook configs invoking the same `hooks/check-token.sh` to automatically refresh expired OAuth tokens before Spotify API calls: `hooks/hooks.json` is Gemini-only (`BeforeTool` — Gemini auto-discovers this fixed path), and `.claude-plugin/hooks.json` is for Claude/Codex (`PreToolUse`), declared explicitly via the `hooks` field in both `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json`. The Claude hooks file lives outside `hooks/` to prevent Claude Code from auto-loading the Gemini `hooks/hooks.json` (which contains `BeforeTool`, an event name Claude rejects). The script detects the platform from the `hook_event_name` in the hook payload. Note the timeout units differ: seconds for `PreToolUse`, milliseconds for `BeforeTool`.
