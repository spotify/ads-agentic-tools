...
```
3. If `ad_product` is `UNSET`, `UNKNOWN`, or not specified, apply the `AUCTION` rules. Validate the new budget values. If any values violate the rules, present a recommended fix and ask the user to either accept the recommendation or provide their own value. Do not auto-correct. Do not execute until validated.
...
curl -s -w "\nHTTP_STATUS:%{http_code}" -H "Authorization: Bearer $TOKEN" \
  ...
  "$BASE_URL/ad_product_catalog"
```
3. If `ad_product` is `UNSET`, `UNKNOWN`, or not specified, apply the `AUCTION` rules. Validate the replacement ad fields against the returned rules. If any values violate the rules, present a recommended fix and ask the user to either accept the recommendation or provide their own value. Do not auto-correct. Do not execute the create request until validated.
