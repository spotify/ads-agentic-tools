...
Route full natural-language campaign builds to the `build-campaign` skill. That skill owns planning, concurrent lookups and estimates, deterministic hierarchy creation, validation, and the separate publish confirmation. Do not duplicate that workflow here.

Use this drafts skill for managing an already-created draft: list, get, edit, validate, publish, delete, or draft-from.
