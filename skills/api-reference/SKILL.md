...
- `GET /ad_product_catalog` — Get validation rules for all externally available ad products (AUCTION, CONTENT, FPMNG). Fetch before creating or updating campaigns, ad sets, and ads to understand constraints and cross-field rules. Cache for 15 minutes. Fetch the parent campaign to determine `ad_product`; when `UNSET`, `UNKNOWN`, or not specified, apply the `AUCTION` rules.
